...
function SuggestedDoctorCard({doctorAgent,setSelectedDoctor,selectedDoctor}:props) {
  return (
    <div className={`flex flex-col items-center  border rounded-2xl shadow hover:border-blue-500 cursor-pointer h-[150px] p-3 ${selectedDoctor?.id == doctorAgent?.id && 'border-blue-500'}`} onClick={()=>setSelectedDoctor(doctorAgent)}>
      ...
      <Image
        ...
        unoptimized={doctorAgent?.image?.startsWith?.('http')}
      />
